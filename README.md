...
| Run hub (`/run-events`, `/run-status`) for HMR reconnect                           | Tag CLI (`pnpm ui visual:tag …`) for bulk skip/review              |
...
3. Serves PNGs with `staticDirs` → `/visual-baselines` (addon preset also mounts
   the snapshot dir; host keeps Tasks/Fava reference `staticDirs`)
4. Passes host `options.visualDelta` in `main.ts` so create/update/run shell the
   UI generator CLIs (not the packaged `visual-delta` bin defaults)
...
Full options table: addon README. Host overrides used here:
...
| `allowRebuild`                | `true` (may `pnpm build-storybook` before run-tests when static is incomplete/stale) |
...
| `GET`  | `/__visual-delta/run-events`                  | Replay / continue an in-flight or recent run (HMR reconnect)     |
| `GET`  | `/__visual-delta/run-status`                  | Lightweight phase/progress JSON for the active/last run          |
| `POST` | `/__visual-delta/cancel-tests`                | Abort in-flight run                                              |
| `POST` | `/__visual-delta/review-status`               | Set CSF review tags (`storyId`+`status`, or batched `updates[]`) |
| `POST` | `/__visual-delta/skip-visual`                 | Add or remove `skip-visual` on a story                           |

**Host coupling:** middleware shells host CLIs via `visualUpdateArgs` /
`visualInteractionUpdateArgs` / `visualTestArgs`. CSF patchers live in the
addon (`story-source`); this host also keeps parallel writers under
`scripts/ui-generator/visual/` for `visual-update` / `visual:tag`.
...
- **Overwrite**: requires approval; sets `PLAYWRIGHT_UPDATE_SNAPSHOTS=1`; resets
  matching review tags → `visual-pending`
- Compare-only runs set `PLAYWRIGHT_UPDATE_SNAPSHOTS=0` and use a **list-only**
  Playwright reporter so Testing Module progress stays live (do not pair
  `--reporter=json` on the same stdout)
- Panel always passes `--allow-dirty` and sets `VISUAL_UPDATE_APPROVED=1` in the
  child env (argv also includes `--approved`)
- After **Update status** / HMR remounts the Testing Module, the client
  reconnects via `/run-status` + `/run-events` instead of losing `Testing N/M`

There is **no** `package.json` script for interaction baselines — only the
middleware `visual-interaction-update` CLI (or invoke the CLI directly).
...
- Host policy: Apps/Beancount and Tasks catalog stories are `skip-visual`
  ...
  compare outside the Playwright suite)
...
| `--skip-build`                             | Prefer reusing `storybook-static` (still rebuilds when incomplete/stale) |
| `--rebuild`                                | Always run `build-storybook` before Playwright writers                   |
...
```bash
pnpm ui visual:tag skip --component button
pnpm ui visual:tag include --component button
pnpm ui visual:tag review --status ready --component button
pnpm ui visual:tag review --status failed --story-id shadcn-button--default
pnpm ui visual:tag skip --prefix apps-beancount-
```

See `pnpm ui visual:tag --help` and [`docs/cli/visual-tag.md`](./docs/cli/visual-tag.md).
Review tags remain mutually exclusive (setting `ready` clears `failed` and the
other review tags on that story).
...
# Agent / docs CLI
pnpm ui                     # CLI entry (guide | components | mcp | visual:tag | …)
...
| `pnpm ui visual:tag …`             | Bulk `skip-visual` / review tags (see `visual:tag --help`) |
...
5. Playwright: compare-only by default (`updateSnapshots: "none"`); serve a
   **complete** `storybook-static` (`index.json` + `iframe.html`) for the suite;
   skip stories tagged `skip-visual`. Prefer a warm static server on the visual
   port and a list-only reporter for live Testing Module progress.
...
| `ui:visual:tag` / `ui visual:tag` | Bulk skip-visual / review tag updates                    |
...
6. Host CLIs: `visual-update`, `visual-interaction-update`, `visual:tag`, CSF
   patchers under `scripts/ui-generator/visual/`
...
9. Complete `storybook-static` (`index.json` + `iframe.html`) before visual runs
...
tag (setting one clears the others — including when the desired tag was already
present alongside a sibling). Accept/Unaccept set approved/pending; the panel
pad toggles ready/failed. Skipping visual clears review tags. Stories without a
committed baseline PNG are never stamped `visual-failed` (Update status skips
them; middleware / `visual:tag review --status failed` refuse). Bulk updates:
`pnpm ui visual:tag` (see Commands / `docs/cli/visual-tag.md`).
