...
export function runSpecificationValidation({
  ...
  return {
    ...
    stats: {
      ...
      surfaces: context.model.coverage.length,
    },
  };
}
...
export function main({ log = console, run = runSpecificationValidation } = {}) {
  try {
    ...
    log.log(
      `Specification validated: ${result.stats.validators} validators, ${result.stats.chapters} chapters, ${result.stats.requirements} requirements, ${result.stats.surfaces} coverage rows.`,
    );
    return 0;
  } catch (error) {
