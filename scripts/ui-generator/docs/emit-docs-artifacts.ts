...
/**
 * Give a newly intaken family a real primary CSF module. Existing curated
 * stories remain authoritative and are never overwritten by docs refreshes.
 *
 * The first runnable upstream example is intentionally only a scaffold: it
 * keeps generated MDX valid during the governed intake transaction and marks
 * the new surface as awaiting visual review. Maintainers can then replace or
 * extend it with interaction-focused stories without fighting regeneration.
 */
function emitPrimaryStory(args: {
  component: string;
  storyTitle: string;
  example: RewrittenExample;
}): string {
  const pascal = toPascalCase(args.component);
  const exampleName = toPascalCase(args.example.example.slug);
  const wrapperClass = args.component === "sidebar" ? "h-[480px] p-0" : "p-4";

  return `<script module lang="ts">
  // @generated by ui-generator — replace with curated coverage after intake
  import { defineMeta } from "@storybook/addon-svelte-csf";
  import * as ${pascal} from "./index.js";
  import * as exampleSources from "./${pascal}.example-sources.js";
  import ${exampleName}Example from "./examples/${args.example.example.slug}.svelte";

  const { Story } = defineMeta({
    title: ${JSON.stringify(args.storyTitle)},
    component: ${storyMetaComponentExpr(args.component, pascal)},
  });
</script>
...
<Story
  ...
  parameters={{
    docs: {
      source: {
        code: exampleSources.${exampleName},
        language: "html",
        type: "code",
      },
    },
...
export function emitDocsArtifacts(args: {
  ...
  if (examples.length > 0) {
    ...
    const primaryStoriesPath = path.join(targetDir, `${pascal}.stories.svelte`);
    if (!existsSync(primaryStoriesPath)) {
      writeFileSync(
        primaryStoriesPath,
        emitPrimaryStory({ component, storyTitle, example: examples[0]! }),
      );
      written.push(primaryStoriesPath);
    }
  }
