...
- The tree MUST retain an accessible label without duplicating the owning leaf title, pressed toolbar actions MUST use the workspace accent foreground, selected tree rows MUST use the same background wash as row hover, and nested indentation guides MUST sit beneath expanded disclosure chevron tips.
