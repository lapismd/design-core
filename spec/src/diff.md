...
- The public boundary is `@lapismd/design-core/diff`.
- The editor MUST report resolved content through `onResolvedChange`, overlay optional editable sides that report `onLeftChange`, `onBaseChange`, and `onRightChange`, point merge actions at the center pane, and reassemble merge blocks when any editable pane is typed so new added, removed, and conflict hunks appear.
