<script lang="ts">
  ...
  function handleTabClick(
    ...
    if (
      ...
      const tab = tabFor(item);
      if (tab && tab.closable !== false) controller.closeTab(tab.id);
      return;
    }
    select(item, index);
  }
  ...
  >
    {#each pane.items as item, index (item.id)}
      ...
      <button
        ...
        aria-pressed={active}
        aria-keyshortcuts={item.kind === "tab" && item.closable !== false
          ? "Delete"
          : undefined}
        ...
        onpointerdown={(event) => tab && dragState.startPointer(event, tab.id)}
      ...
      >
        <span class="ui-workspace-stacked-tabs__tab-inner">
          <span class="ui-workspace-stacked-tabs__tab-icon">
