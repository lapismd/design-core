...
- Explorer file activation now reports application-independent `current`,
  `reveal-or-new-tab`, and forced `new-tab` dispositions. A cancellable
  single-click delay prevents double-click from first dispatching `current`;
  modifier and middle clicks remain immediate. The tree keeps its accessible
  Files name without repeating the leaf title, and the pressed auto-reveal
  action uses the theme accent.
