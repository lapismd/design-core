...
function ensureDynamicHighlightType(tokenType: string): void {
  ...
  if (!dynamicStyleSheet) {
    ...
    style.setAttribute("data-ui-code-highlight-dynamic", "");
  ...
  const name = `ui-code-${tokenType}`;
  const colorVar = `var(--ui-code-syntax-${tokenType}, currentColor)`;
  dynamicStyleSheet.insertRule(
    `[data-ui-component="code-block"] code::highlight(${name}), [data-ui-component="code-block"] code::highlight(${name}) { color: ${colorVar}; }`,
  );
}
