...
export const CustomComposition =
  '<ColumnCanvas.Column id="tasks">\n\t<ColumnCanvas.Header>\n\t\t<ColumnCanvas.Title>Tasks</ColumnCanvas.Title>\n\t\t<ColumnCanvas.Count>{tasks.length}</ColumnCanvas.Count>\n\t\t<ColumnCanvas.HeaderActions>\n\t\t\t<ColumnCanvas.Toggle aria-label="Collapse Tasks column" />\n\t\t\t<ColumnCanvas.Close aria-label="Close Tasks column" />\n\t\t</ColumnCanvas.HeaderActions>\n\t</ColumnCanvas.Header>\n\n\t<ColumnCanvas.Body onScrollNearEnd={loadMore}>\n\t\t{#each tasks as task (task.id)}\n\t\t\t<ColumnCanvas.Item\n\t\t\t\tselected={canvas.isSelected(0, task.id)}\n\t\t\t\tonclick={() => canvas.select(0, task.id)}\n\t\t\t>\n\t\t\t\t{task.title}\n\t\t\t</ColumnCanvas.Item>\n\t\t{/each}\n\t</ColumnCanvas.Body>\n</ColumnCanvas.Column>';
