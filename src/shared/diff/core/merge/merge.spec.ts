...
function blockKinds(
  ...
  return assembleThreeWayMerge(left, base, right).blocks.map(
    (block) => block.kind,
  );
}

function renderComponent(
  model: ReturnType<typeof assembleThreeWayMerge>,
  side: "left" | "base" | "right",
  blockId: string,
): RenderComponent {
  const component = createMergeRenderModel(model).sides[side].find(
...
test("assembles one-way modified blocks", () => {
  ...
  assert.equal(
    model.blocks.some((block) => block.kind === "modified"),
    true,
  );
...
test("adds modified-line overlays", () => {
  ...
  assert.ok(leftLine);
  assert.equal(
    leftLine.parts.some((part) => part.changed),
    true,
  );
});
...
test("assembles one-way unchanged content", () => {
  ...
  assert.equal(
    model.blocks.every((block) => block.kind === "unchanged"),
    true,
  );
...
test("assembles one-way added and removed content", () => {
  ...
  assert.deepEqual(
    ...
    ["added", "unchanged", "unchanged", "unchanged", "removed"],
  );
...
test("assembles the C quicksort fixture without stalling", () => {
  ...
  assert.equal(
    model.blocks.every((block) => block.sides.base !== undefined),
    true,
  );
});
...
test("creates side-major render components and connector lanes for the C fixture", () => {
  ...
  assert.equal(
    renderModel.leftConnections.every(
      (connection) =>
        connection.fromSide === "left" && connection.toSide === "base",
    ),
    true,
  );
  assert.equal(
    renderModel.rightConnections.every(
      (connection) =>
        connection.fromSide === "base" && connection.toSide === "right",
    ),
    true,
  );
});
...
test("places merge and resolve actions on render components", () => {
  ...
  const left = renderModel.sides.left.find(
    (component) => component.blockId === conflictBlock.id,
  );
...
test("places merge actions on one-way changed render components", () => {
  const modified = createMergeRenderModel(
    assembleOneWayMerge("a\nleft\nc\n", "a\nright\nc\n"),
    ["left", "right"],
  );
  const modifiedLeft = modified.sides.left.find(
    (component) => component.blockKind === "modified",
  );
  const modifiedRight = modified.sides.right.find(
    (component) => component.blockKind === "modified",
  );
  ...
  const added = createMergeRenderModel(
    assembleOneWayMerge("a\nc\n", "a\nright\nc\n"),
    ["left", "right"],
  );
  const addedLeft = added.sides.left.find(
    (component) => component.blockKind === "added",
  );
  const addedRight = added.sides.right.find(
    (component) => component.blockKind === "added",
  );
  ...
  const removed = createMergeRenderModel(
    assembleOneWayMerge("a\nleft\nc\n", "a\nc\n"),
    ["left", "right"],
  );
  const removedLeft = removed.sides.left.find(
    (component) => component.blockKind === "removed",
  );
  const removedRight = removed.sides.right.find(
    (component) => component.blockKind === "removed",
  );
...
test("uses delete actions for source content already merged into center", () => {
  ...
  const conflict = model.blocks.find((block) => block.kind === "conflict");
  assert.ok(conflict);
  const merged = applyMergeAction(model, {
    type: "accept-left",
    blockId: conflict.id,
  });
  const renderModel = createMergeRenderModel(merged);
  const left = renderModel.sides.left.find(
    (component) => component.blockId === conflict.id,
  );
...
test("marks the picked source and center as added and the unpicked modified side as removed", () => {
  ...
  const renderModel = createMergeRenderModel(model);
  const left = renderModel.sides.left.find(
    (component) => component.blockId === block.id,
  );
  const base = renderModel.sides.base.find(
    (component) => component.blockId === block.id,
  );
...
test("merging both conflict sides stacks left then right in center content", () => {
  ...
  const conflict = model.blocks.find(
    (candidate) => candidate.kind === "conflict",
  );
  assert.ok(conflict);
  const withLeftMerged = mergeRenderComponentIntoCenter(
    model,
    renderComponent(model, "left", conflict.id),
  );
  const withRightMerged = mergeRenderComponentIntoCenter(
    withLeftMerged,
    renderComponent(withLeftMerged, "right", conflict.id),
  );
...
test("merging both conflict sides from right first still stacks left then right in center content", () => {
  ...
  const withRightMerged = mergeRenderComponentIntoCenter(
    model,
    renderComponent(model, "right", conflict.id),
  );
  const withLeftMerged = mergeRenderComponentIntoCenter(
    withRightMerged,
    renderComponent(withRightMerged, "left", conflict.id),
  );
...
test("removing one side from stacked conflict content keeps the other selected side", () => {
  ...
  const withBothMerged = mergeRenderComponentIntoCenter(
    withLeftMerged,
    renderComponent(withLeftMerged, "right", conflict.id),
  );
