...
import { diffInlineParts } from "./inline-diff.js";
import { hasTrailingNewline, joinLines } from "./lines.js";
...
export function createBlock(
  ...
  for (const side of ["left", "base", "right"] as const) {
    ...
    sides[side] = createLines(
      id,
      side,
      sideLines,
      lineStarts[side] ?? null,
      compareLines,
      options,
    );
  }
...
export function recomputeModel(
  model: MergeModel,
  blocks: MergeBlock[],
): MergeModel {
  ...
  const unresolvedConflictCount = blocks.filter(
...
export function cloneBlockWithBaseLines(
  ...
  const lineStart = block.sides.base?.[0]?.lineNumber
    ? block.sides.base[0].lineNumber - 1
    : null;
  const nextBlock = createBlock(
    ...
    {
      left: block.sides.left?.[0]?.lineNumber
        ? block.sides.left[0].lineNumber - 1
        : null,
      base: lineStart,
      right: block.sides.right?.[0]?.lineNumber
        ? block.sides.right[0].lineNumber - 1
        : null,
    },
    options,
  );
  return {
    ...nextBlock,
    originalBaseLines: [
      ...(block.originalBaseLines ?? linesFromBlock(block, "base")),
    ],
  };
}
