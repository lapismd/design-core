...
import { createBlock, recomputeModel } from "./blocks.js";
import { diffSequences } from "./diff.js";
import { linesEqual, splitLines } from "./lines.js";
...
function findInsertion(
  ...
  return (
    changes.find(
      (change) =>
        change.baseStart === baseIndex && change.baseEnd === baseIndex,
    ) ?? null
  );
}

function findCoveringChange(
  changes: readonly SideChange[],
  baseIndex: number,
): SideChange | null {
  return (
    changes.find(
      (change) => change.baseStart <= baseIndex && baseIndex < change.baseEnd,
    ) ?? null
  );
}

function sideLinesForRange(
  change: SideChange | null,
  baseLines: readonly string[],
  start: number,
  end: number,
): string[] {
...
function createThreeWayChangeBlock(
  ...
  const leftResolvedLines = sideLinesForRange(
    leftChange,
    baseLines,
    start,
    end,
  );
  const rightResolvedLines = sideLinesForRange(
    rightChange,
    baseLines,
    start,
    end,
  ...
  const leftChanged =
    Boolean(leftChange) &&
    !linesEqual(leftResolvedLines, originalBaseLines, options);
  const rightChanged =
    Boolean(rightChange) &&
    !linesEqual(rightResolvedLines, originalBaseLines, options);
...
export function assembleThreeWayMerge(
  left: string,
  base: string,
  right: string,
  ...
  while (baseIndex <= baseLines.length) {
    ...
    const end = Math.max(
      leftChange?.baseEnd ?? baseIndex + 1,
      rightChange?.baseEnd ?? baseIndex + 1,
    );
    createThreeWayChangeBlock(
      blocks,
      baseLines,
