...
import { normalizeComparableLine } from "./options.js";
...
export function diffSequences(
  ...
  const leftComparable = left.map((line) =>
    normalizeComparableLine(line, options),
  );
  const rightComparable = right.map((line) =>
    normalizeComparableLine(line, options),
  );
