import type { MergeAction, MergeModel } from "./types.js";
import {
...
import { assembleOneWayMerge } from "./one-way.js";
import { assembleThreeWayMerge } from "./three-way.js";
  ...
  if (model.blocks.length === 0) {
    const blocks = [
      createBlock(
        ...
        "unchanged",
        true,
        { left: [], base: nextLines, right: [] },
        { left: null, base: 0, right: null },
        model.options,
      ),
    ];
...
export function applyMergeAction(
  ...
  if (action.type === "edit-center") {
    return model.mode === "three-way"
      ? replaceCenterContent(model, action.content)
    ...
    action.type === "accept-left"
      ...
      : action.type === "accept-right"
        ...
        : action.type === "accept-side" || action.type === "accept-all"
          ...
          : null;
  const applyToAll =
    action.type === "accept-all" ||
    (targetSide !== null && !("blockId" in action));
  const blocks = model.blocks.map((block) => {
    const isTarget =
      applyToAll || ("blockId" in action && action.blockId === block.id);
    if (!isTarget) {
      return block;
    }
    if (targetSide) {
      return cloneBlockWithBaseLines(
        block,
        linesFromBlock(block, targetSide),
        true,
        model.options,
      );
    }
