...
| Rules            | ✅ loaded      | 🟡 collection + history | Fava sortable rule collection, header apply/add callbacks, active callback, host menu, and run-history model compose. `RuleList` now uses Fava's 56rem collection measure, 24px route inset, compact 48px muted column header, and a single-line 70px rule row using normalized tokens and shared controls; `LedgerSettingsNavigation` selects Rules in the shared settings shell, while the shared frame renders Fava's trailing rule-count metadata through `Badge`. The current reference-screen delta is 0.838% (down from 1.376% before the collection/settings slices). Its interaction test restores the active rule state before capture; add production adapters, then review Visual Delta ≤0.1%.                                                  |
