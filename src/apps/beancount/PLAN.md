...
| Income statement | ✅             | 🟡 partial              | Fava toolbar, fixed 250px measured stacked-bars plot, source-compatible insets, bottom perspective tabs, and a separate Income hierarchy compose. The protected-reference candidate delta is 8.579% (down from 9.642%); align sidebar/report control density, production data, and remaining statement sections, then review Visual Delta ≤0.1%.                                                                                                                                                                                                                                                                                                                                                                                                            |
| Balance sheet    | ✅             | 🟡 partial              | Fava report-scroll geometry, toolbar, fixed 250px unframed stepped net-worth history, source-compatible axis cadence, bottom perspective tabs, and the Assets/Liabilities section boundary now compose through normalized tokens. The protected-reference candidate delta is 1.956% (down from 3.797%); align the production data adapter and full liabilities/equity display models, then review Visual Delta ≤0.1%.                                                                                                                                                                                                                                                                                                                                       |
| Trial balance    | ✅             | 🟡 partial              | Fava toolbar, unframed Fava-height treemap with its HCL-derived normalized reference colour, bottom perspective tabs, and a separate contribution ledger compose. The protected-reference candidate delta is 4.107% (down from 4.376%); align sidebar/control density, production data adapter, and lower table section, then review Visual Delta ≤0.1%.                                                                                                                                                                                                                                                                                                                                                                                                    |
| Account detail   | ✅             | 🟡 partial              | Fava breadcrumb/content-scroll separation, 8px report/table insets, fixed 250px stepped balance history, source-compatible axis, bottom chart-history perspectives, left-aligned account-table tabs, and account-activity fixture now compose. The protected-reference candidate delta is 2.580% (down from 2.912%); align activity density and full account history, then review Visual Delta ≤0.1%.                                                                                                                                                                                                                                                                                                                                                       |
...
| Query            | ✅             | 🟡 partial              | The Fava-reference screen holds its blank BQL-entry state with the zero-ledger sidebar fixture. `QueryComposer` now follows Fava's compact muted command field, 28px grouped secondary actions, `LetterText` formatter, and `Play` execution icon; `QueryWorkspace` continues to compose the command, host error, and saved-query disclosure. BQL execution, history persistence, route selection, and result rendering stay in the adapter. Add that production adapter, then review Visual Delta ≤0.1%.                                                                                                                                                                                                                                                   |
| Errors           | ✅             | 🟡 partial              | Fava no-records panel now keeps the Grid alert's 8px route inset, 16px top separation, compact 14px line rhythm, and alert padding/radius; parent-ledger navigation state, deterministic validation-failure composition, and editor-navigation callback remain controlled. Align the production error adapter, then review Visual Delta ≤0.1%.                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| Sources          | ✅             | 🟡 expanded details     | Fava connected/available source cards, controlled expanded setup/field details, source toolbar, and host callbacks compose. The shared Settings sidebar selects Bank Sync rather than rendering the ledger-tree fixture; the collapsed catalog now uses Fava's 48rem measure, 24px route inset, 64px connection row, 176px available cards, compact heading/name rhythm, and separate connected/available provider-mark roles through normalized tokens. The current reference-screen delta is 1.586% (down from 3.074%). Its interaction test returns to the collapsed, YAML-off reference state before capture; add live sync adapters, then review Visual Delta ≤0.1%.                                                                                   |
| Import accounts  | ✅ loaded      | 🟡 expanded details     | Fava source-account and unassigned rows, controlled expanded account groups, Sync all, and host actions compose. The shared Settings sidebar selects Accounts; the collapsed catalog now uses Fava's 48rem measure, 24px route inset, 64px rows, compact 14px names and 12px status copy, 36px provider marks, and pale negative provider role through normalized tokens. The current reference-screen delta is 0.797% (down from 1.629%). Its interaction test resets its collapsed capture state; add production setup adapters, then review Visual Delta ≤0.1%.                                                                                                                                                                                          |
| Rules            | ✅ loaded      | 🟡 collection + history | Fava sortable rule collection, header controls, active callback, host menu, and run-history model compose. `RuleList` uses Fava's 56rem measure, 24px route inset, compact 48px header, single-line 70px rule row, and 12px/14px empty-history rhythm with normalized tokens. `LedgerSettingsNavigation` selects Rules in the shared settings shell, while the shared frame renders Fava's trailing rule-count metadata through `Badge`. The current reference-screen delta is 0.748% (down from 1.376% before the collection/settings slices). Its interaction test restores the active rule state before capture; add production adapters, then review Visual Delta ≤0.1%.                                                                                |
...
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------- |
| Catalog surface | 74 Beancount components and 61 colocated story modules, including controlled `LedgerSettingsNavigation`                                    | Every promoted surface has a display-model contract and a tested story                    |
