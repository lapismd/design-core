...
| Rules            | ✅ loaded      | 🟡 collection + history | Fava sortable rule collection, header apply/add callbacks, active callback, host menu, and run-history model compose. `RuleList` now uses Fava's 56rem collection measure, 24px route inset, compact 48px muted column header, and a single-line 70px rule row using normalized tokens and shared controls; `LedgerSettingsNavigation` selects Rules in the shared settings shell. Together, those slices reduced the reference-screen delta from 1.376% to 0.837%. Its interaction test restores the active rule state before capture; add production adapters, then review Visual Delta ≤0.1%.                                                                                                                                                            |
